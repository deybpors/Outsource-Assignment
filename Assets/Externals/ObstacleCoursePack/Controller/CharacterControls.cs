...
public class CharacterControls : MonoBehaviour {
	...
	public Transform particleSpawnPoint; // feet position (optional)

	private bool wasGrounded;
	[HideInInspector] public bool groundedNow;
	private bool jumped;
	...
	{
		if (landingParticle == null) return;

		Vector3 spawnPos = particleSpawnPoint != null
			? particleSpawnPoint.position
			: transform.position;

		Instantiate(landingParticle, spawnPos, Quaternion.identity);
	}
		...
		if (canMove)
		{
			...
			if (groundedNow)
			{
			 // Calculate how fast we should be moving
				...
				// Jump
				if (groundedNow && Input.GetButton("Jump"))
				{
					jumped = true;
